...
DeBERTa（Decoding-enhanced BERT with Disentangled Attention）是一个在BERT架构基础上提出的改进型预训练语言模型。DeBERTa的核心改进集中在更有效的表示和解码策略上。DeBERTa的不同版本（v1, v2, v3）各自引入了一些新的特性和优化，下面是它们的主要改进点：
